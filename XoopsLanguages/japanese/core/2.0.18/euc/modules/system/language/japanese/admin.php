<?php
//%%%%%%	File Name  admin.php 	%%%%%
define("_MD_AM_DBUPDATED","�ǡ����١����򹹿����ޤ���");


define('_MD_AM_CONFIG','System Configuration');


// Admin Module Names
define("_MD_AM_ADGS","���롼�״���");
define("_MD_AM_BANS","�Хʡ�����");
define("_MD_AM_BKAD","�֥�å�����");
define("_MD_AM_MDAD","�⥸�塼�����");
define("_MD_AM_SMLS","�饢����������");
define("_MD_AM_RANK","�桼����󥭥�����");
define("_MD_AM_USER","�桼������");
define("_MD_AM_FINDUSER","�桼������");
define("_MD_AM_PREF","��������");
define("_MD_AM_VRSN","�С������");
define("_MD_AM_MLUS","�桼�����˥᡼������");
define('_MD_AM_IMAGES','���᡼�����ޥͥ��㡼');
define('_MD_AM_AVATARS','���Х������ޥͥ��㡼');
define('_MD_AM_TPLSETS','�ƥ�ץ졼��');
define('_MD_AM_COMMENTS','������');

// Group permission phrases
define('_MD_AM_PERMADDNG','���롼�ס��ѡ��ߥå������ɲä˼��Ԥ��ޤ����ʥѡ��ߥå����̾��%s �оݥ����ƥࡧ%s �оݥ��롼�ס�%s��');
define('_MD_AM_PERMADDOK','���롼�ס��ѡ��ߥå������ɲä��ޤ����ʥѡ��ߥå����̾��%s �оݥ����ƥࡧ%s �оݥ��롼�ס�%s��');
define('_MD_AM_PERMRESETNG','��%s�ץ⥸�塼��Υ��롼�ס��ѡ��ߥå��������ν�����˼��Ԥ��ޤ���');
define('_MD_AM_PERMADDNGP','���Υ����ƥ�ξ�̥����ƥ����Ƥ˥ѡ��ߥå�����Ϳ����ɬ�פ�����ޤ�');

?>