...
define($constpref."_DESC","Moduł zabezpieczający Xoopsa, przed różnymi rodzajami ataków z sieci, takich jak: DoS , SQL Injection i skażeniem zmiennych.");

// Menu
define($constpref."_ADMININDEX","Centrum Zabezpieczeń");
define($constpref."_ADVISORY","Doradca Bezpieczeństwa");
...
define($constpref.'_DEFAULT_LANGDSC','Wybierz język wyświetlania komunikatów zanim uruchomisz common.php');
...
define($constpref.'_BANIP_TIME0','Czas na jaki są zawieszane zbanowane adresy IP (sek.)');

define($constpref.'_LOGLEVEL0','Bez logów');
define($constpref.'_LOGLEVEL15','Mało logów');
define($constpref.'_LOGLEVEL63','Dokładne logi');
define($constpref.'_LOGLEVEL255','Pełne logi');
...
define($constpref.'_DIE_NULLBYTE','WyjdĽ jeśli stwierdzone zostaną puste bajty');
...
define($constpref.'_DIE_BADEXT','WyjdĽ jeśli wgrywane są podejrzane pliki (tak/nie)');
...
define($constpref.'_CONTAMI_ACTION','Działanie w przypadku wykrycia próby skażenia zmiennych');
...
define($constpref.'_ISOCOM_ACTION','Działanie w przypadku wykrycia izolowanego otwarcia komentarza.');
...
define($constpref.'_BF_COUNT','Ochrona przed atakami siłowymi (Brute Force)');
define($constpref.'_BF_COUNTDSC','Tutaj możesz określić liczbę dopuszczalnych prób zalogowania w ciągu 10 minut. Jeśli ktoś poda złe dane więcej razy, niż wynosi limit, jego IP zostanie zbanowane. Ta funkcja chroni przed próbami złamania haseł dostępu metodą prób i błędów. Domyślnie ustawiona wartość wynosi 10.');

define($constpref.'_BWLIMIT_COUNT','Ograniczenie transferu');
define($constpref.'_BWLIMIT_COUNTDSC','Określa maksymalną liczbę odwołań do pliku mainfile.php podczas zadanej ramki czasowej. Ta wartość powinna być równa 0 dla normalnych środowisk które nie są przeciążone. Wartości mniejsze niż 10 będą ignorowane.');

define($constpref.'_DOS_SKIPMODS','Moduły wyłączone z ochrony przed DoS/Crawler');
...
define($constpref.'_DOS_EXPIRE','Ramka czasowa masowych odsłon (w sek.)');
define($constpref.'_DOS_EXPIREDSC','Ta wartość określa czas obserwowania licznych/częstych odsłon (wielokrotne odświeżanie - atak F5) i robaków zajmujących transfer. Domyślnie 60 sekund. .');
...
define($constpref.'_DOSOPT_BIP','Banuj IP (bez limitu czasowego)');
define($constpref.'_DOSOPT_BIPTIME0','Banuj IP (limit - moratorium)');
...
define($constpref.'_DBLAYERTRAP','Włącz warstwę DB wyłapującą ataki SQL Injection');
define($constpref.'_DBLAYERTRAPDSC','Prawie wszystkie ataki SQL Injection będą udaremnione przez tę funkcję. Ta fukcja wymaga wsparcia ze strony interfejsu bazy danych (database factory). Więcej możesz się dowiedzieć na stronach Doradcy Bezpieczeństwa. To ustawienie musi być włączone. Nie wyłączaj go bez ważnych powodów.');
...
define($constpref.'_BIGUMBRELLADSC','Ta funkcja chroni przed niektórymi atakami XSS (cross-site scripting). Nie ma jednak 100% skuteczności. Domyślnie jest wyłączona. Włączenie jej to raczej niezły pomysł.');
