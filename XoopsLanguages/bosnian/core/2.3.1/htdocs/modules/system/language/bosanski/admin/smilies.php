<?php
// $Id: smilies.php,v 1.5 2003/02/22 19:39:20 w4z004 Exp $
//%%%%%%	Admin Module Name  Smilies 	%%%%%
define('_AM_DBUPDATED',_MD_AM_DBUPDATED);

define('_AM_SMILESCONTROL','Smili-Upravljanje');
define('_AM_CODE','Code');
define('_AM_SMILIE','Smili');
define('_AM_ACTION','Akcija');
define('_AM_EDIT','Uredi');
define('_AM_DEL','Izbri&scaron;i');
define('_AM_CNRFTSD','Smilis se nemo&#382;e u Bazu podataka unijeti.');
define('_AM_ADDSMILE','Smili dodaj');
define('_AM_EDITSMILE','Smili uredi');
define('_AM_SMILECODE','Smili-Code:');
define('_AM_SMILEURL','Slike-URL:');
define('_AM_SMILEEMOTION','Opis:');
define('_AM_ADD','Dodaj');
define('_AM_SAVE','Osiguraj');
define('_AM_WAYSYWTDTS','Pa&#382;nja: &#381;eli&scaron; li Smilie stvarno izbrisati?');
define('_AM_DISPLAYF','U formularu prika&#382;i?');
?>