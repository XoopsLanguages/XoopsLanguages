<?php
// $Id: admin.php 2 2005-11-02 18:23:29Z skalpa $
//%%%%%%	File Name  admin.php 	%%%%%
define("_AD_NORIGHT","Du är inte behörig att komma åt den här avdelningen");
define("_AD_ACTION","Åtgärd");
define("_AD_EDIT","Ändra");
define("_AD_DELETE","Radera");
define("_AD_LASTTENUSERS","10 senaste registrerade besökarna");
define("_AD_NICKNAME","Alias");
define("_AD_EMAIL","E-post");
define("_AD_AVATAR","Avatar");
define("_AD_REGISTERED","Registrerad"); //Registered Date
define('_AD_PRESSGEN', 'Det här är första gången du går in i administrationssektionen. Klicka på knappen för att fortsätta.');
define('_AD_LOGINADMIN', 'Loggar in dig...');
?>
