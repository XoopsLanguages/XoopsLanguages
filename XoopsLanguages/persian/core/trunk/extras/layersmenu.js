...
//Start open popup at the right corner in the left mouse over - hacked by irmtfan www.jadoogaran.org
function setleft(layer,x) {
	if (DOM) {
        	document.getElementById(layer).style.right = x;
		document.getElementById(layer).style.right = x + 'px';
        } else if (NS4) {
		document.layers[layer].right = x;
	} else {
		document.all[layer].style.pixelright = x;
	}
}
//End open popup at the right corner in the left mouse over - hacked by irmtfan www.jadoogaran.org
