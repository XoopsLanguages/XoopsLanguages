...
xoops_smallworld.widget( "ui.geo_autocomplete_now", {
	...
	options: {
		...
		source: function(_request, _response) {
		  ...
		  // Added a select so lat,lng and short_country name is based of selected item (if more than one is suggested) (culex)	
			...
			var c_code = codeLatLngNow(ui.item.geometry.lat(),ui.item.geometry.lng());
			 var cities = ui.item.value;
			/* xoops_smallworld("input[name=present_city]").val(cities);  */
			xoops_smallworld("input[name=present_country]").val(c_code);
			xoops_smallworld("input[name=present_lat]").val(ui.item.geometry.lat());
			xoops_smallworld("input[name=present_lng]").val(ui.item.geometry.lng());
		},
...
function codeLatLngNow(lat,lng,txt) {
	var txt;
