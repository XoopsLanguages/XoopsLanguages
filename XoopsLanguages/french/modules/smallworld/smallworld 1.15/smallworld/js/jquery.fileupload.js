...
(function (xoops_smallworld) {
    ...
    FileUpload = function (container) {
        var fileUpload = this,
            ...
            initEventHandlers = function () {
                if (settings.dragDropSupport) {
                    ...
                    xoops_smallworld(document).bind(documentListeners);
            ...
            removeEventHandlers = function () {
                xoops_smallworld.each(documentListeners, function (key, value) {
                    xoops_smallworld(document).unbind(key, value);
                });
                xoops_smallworld.each(dropZoneListeners, function (key, value) {
            ...
            handleGlobalProgress = function (event, files, index, xhr, settings) {
                ...
                if (event.lengthComputable && typeof settings.onProgressAll === func) {
                    ...
                    xoops_smallworld.each(loaderList, function (index, item) {
            ...
            getFormData = function (settings) {
                ...
                } else if (xoops_smallworld.isArray(settings.formData)) {
                    return settings.formData;
                } else if (settings.formData) {
                    formData = [];
                    xoops_smallworld.each(settings.formData, function (name, value) {
            ...
            isSameDomain = function (url) {
                if (protocolRegExp.test(url)) {
                    ...
                    if ((index === indexStart || index === url.indexOf('@', indexStart) + 1) &&
                            (url.length === pathIndex || xoops_smallworld.inArray(url.charAt(pathIndex), ['/', '?', '#']) !== -1)) {
                        return true;
                    }
                    return false;
                }
            ...
            initUploadRequest = function (files, index, xhr, settings) {
                ...
                if (xoops_smallworld.isArray(settings.requestHeaders)) {
                    xoops_smallworld.each(settings.requestHeaders, function (index, header) {
                        xhr.setRequestHeader(header.name, header.value);
                    });
                } else if (settings.requestHeaders) {
                    xoops_smallworld.each(settings.requestHeaders, function (name, value) {
            ...
            formDataUpload = function (files, xhr, settings) {
                var formData = new FormData(),
                    i;
                xoops_smallworld.each(getFormData(settings), function (index, field) {
            ...
            buildMultiPartFormData = function (boundary, files, filesFieldName, fields) {
                var doubleDash = '--',
                    ...
                    formData = '',
                    buffer = [];
                xoops_smallworld.each(fields, function (index, field) {
                ...
                send = function () {
                    ...
                    if (!settings.multipart) {
                        if (xhr.upload) {
                            ...
                            xoops_smallworld.error('Browser does not support XHR file uploads');
                        }
                    } else {
                        ...
                        if (typeof FormData !== undef) {
                            ...
                            xoops_smallworld.error('Browser does not support multipart/form-data XHR file uploads');
                        }
                    }
                };
                if (!nextChunk) {
            ...
            handleUpload = function (event, files, input, form, index) {
                var xhr = new XMLHttpRequest(),
                    uploadSettings = xoops_smallworld.extend({}, settings);
            ...
            legacyUploadFormDataInit = function (input, form, settings) {
                ...
                xoops_smallworld.each(formData, function (index, field) {
                    xoops_smallworld('<input type="hidden"/>')
                ...
                send = function () {
                    ...
                    iframe
                        ...
                        .bind('load', function (e) {
                            ...
                            xoops_smallworld('<iframe src="javascript:false;" style="display:none;"></iframe>')
            ...
            handleLegacyUpload = function (event, input, form, index) {
                if (!(event && input && form)) {
                    xoops_smallworld.error('Iframe based File Upload requires a file input change event');
                    return;
                }
                // javascript:false as iframe src prevents warning popups on HTTPS in IE6:
                var iframe = xoops_smallworld('<iframe src="javascript:false;" style="display:none;" name="iframe_' +
                ...
                xoops_smallworld.each(files, normalizeFile);
            ...
            replaceFileInput = function (input) {
                ...
                xoops_smallworld('<form/>').append(inputClone).get(0).reset();
        ...
        this.onChange = function (e) {
            ...
            var input = xoops_smallworld(e.target),
                form = xoops_smallworld(e.target.form);
            if (form.length === 1) {
        ...
        this.init = function (options) {
            ...
            if (container.data(settings.namespace)) {
                xoops_smallworld.error('FileUpload with namespace "' + settings.namespace + '" already assigned to this element');
                return;
            }
            container
        ...
        this.options = function (options) {
            ...
            if (typeof options === undef) {
                return xoops_smallworld.extend({}, settings);
            }
            if (optionsReference) {
                xoops_smallworld.extend(optionsReference, options);
            }
            removeEventHandlers();
            xoops_smallworld.each(options, function (name, value) {
                switch (name) {
                case 'namespace':
                    xoops_smallworld.error('The FileUpload namespace cannot be updated.');
                    return;
    ...
    methods = {
        init : function (options) {
            return this.each(function () {
                (new FileUpload(xoops_smallworld(this))).init(options);
            });
        },
        
        option: function (option, value, namespace) {
            namespace = namespace ? namespace : defaultNamespace;
            var fileUpload = xoops_smallworld(this).data(namespace);
            if (fileUpload) {
                if (!option) {
                ...
                xoops_smallworld.error('No FileUpload with namespace "' + namespace + '" assigned to this element');
            }
            return this.each(function () {
                var fu = xoops_smallworld(this).data(namespace);
                if (fu) {
    ...
    xoops_smallworld.fn.fileUpload = function (method) {
        if (methods[method]) {
            return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof method === 'object' || !method) {
            return methods.init.apply(this, arguments);
        } else {
            xoops_smallworld.error('Method "' + method + '" does not exist on jQuery.fileUpload');
        }
    };
