<?php
/*
 * Suico theme
 * In memory of Marcello Brandao
 * @copyright       The XOOPS Project http://sourceforge.net/projects/xoops/
 * @license         http://www.fsf.org/copyleft/gpl.html GNU public license 3.0
 * @package         themes
 * @since           2.5.x
 * @author          Kris <http://www.xoofoo.org/>
 * @maintained      The XOOPS Project http://sourceforge.net/projects/xoops/
 *
 * @version         $Id
*/

// system_notification_select.html
define("THEME_SHOWHIDE_NOTIFICATION",	"نمایش / مخفی کردن");
define("THEME_SHOWHIDE_NOTIFICATION_DESC",	"نمایش مخفی کردن سیستم آگاهی رسانی");
define("THEME_RSS",						"خوراک");
define("THEME_SEARCH",					"جستجو");
define("THEME_DESC_SEARCH",				"جستجو در این سایت");
define("THEME_KEYWORDS",				"بر اساس کلمات");

// blockszone and centerblocks.html
define("THEME_THISBLOCK_EDIT",			"ویرایش این بلاک");
define("THEME_VIEWALL",					"نمایش همه");

// xo_globalnav.html
define("THEME_NAVTITLE",				"آگاهی ‌ها");

define("THEME_HOME",					"خانه");
define("THEME_HOME_DESC",				"برو به خانه");

define("THEME_MODULE1",					"اخبار");
define("THEME_MODULE1_DESC",			"دیدن اخبار");

define("THEME_MODULE2",					"انجمن");
define("THEME_MODULE2_DESC",			"بازدید از انجمن ‌ها ما");

define("THEME_MODULE3",					"گالری");
define("THEME_MODULE3_DESC",			"بازدید از گالری عکس ما");

define("THEME_MODULE4",					"فیلم ‌ها");
define("THEME_MODULE4_DESC",			"دیدن فیلم ‌ها ما");

define("THEME_MODULE5",					"لینک ‌های وب");
define("THEME_MODULE5_DESC",			"دیدن لینک ‌های وب ما");

define("THEME_MODULE6",					"دریافت فایل");
define("THEME_MODULE6_DESC",			"دریافت فایل ‌های ما");

define("THEME_MODULE7",					"");
define("THEME_MODULE7_DESC",			"");

define("THEME_MODULE8",					"");
define("THEME_MODULE8_DESC",			"");

define("THEME_MODULE9",					"");
define("THEME_MODULE9_DESC",			"");

// module name in xo_globalnav.html  anomymous mode
define("THEME_MODULE1a",				"دستاندرکاران");
define("THEME_MODULE1a_DESC",			"دستاندرکاران وب سایت");
define("THEME_MODULE2a",				"بومی سازی");
define("THEME_MODULE2a_DESC",			"Localize us on a map");

define("THEME_CONTACT",					"تماس");
define("THEME_CONTACT_DESC",			"تماس با ما؟");
define("THEME_SITEMAP",					"نقشه سایت");

// xo_userbar.html
define("THEME_REGISTER",				"ثبت نام");
define("THEME_REMEMBER",				"مرا با یاد داشته باش؟");
define("THEME_PSEUDO",					"نام کاربری");
define("THEME_DESC_PSEUDO",				"نام کاربری خود را بنویسید");
define("THEME_PASSWORD",				"پسورد");
define("THEME_DESC_PASSWORD",			"واژه رمز خود را بنویسید");
define("THEME_CONNECT",					"اتصال");
define("THEME_LOSTACCOUNT",				"واژه رمز را فراموش کرده اید؟"); // add for suico theme
define("THEME_CLOSEPOPUP",				"بستن"); // add for suico theme

define("THEME_WELLCOME",				"خوش آمدید");
define("THEME_PROFILE",					"نمایش ویژگی ‌های فردی");
define("THEME_NOTIFICATION",			"نمایش آگاهی ‌ها");
define("THEME_MESSAGE",					"نمایش پیام ‌ها");
define("THEME_YOUHAVE",					"شما ");
define("THEME_NOTREAD",					"پیام ‌های خوانده نشده دارید");
define("THEME_MYACCOUNT",				"حساب  من");
define("THEME_PRIVATE",					"دسترسی خصوصی");

define("THEME_UFROM",					"محل"); // add for suico theme
define("THEME_UOCCUPAT",				"شغل"); // add for suico theme
define("THEME_UINTEREST",				"علاقه‌مندی‌‌ها"); // add for suico theme
define("THEME_UINFOS",					"اطلاعات"); // add for suico theme
define("THEME_UREGDATE",				"عضویت از"); // add for suico theme
define("THEME_UPOSTS",					"تعداد ارسال ‌ها"); // add for suico theme
define("THEME_ULASTLOGIN",				"آخرین ورود"); // add for suico theme

define("THEME_UEDITPROFILE",			"ویرایش ویژگی ‌های فردی"); // add for suico theme
define("THEME_UEDITAVATAR",				"عوض کردن تصویر"); // add for suico theme
define("THEME_UEDITPASSWORD",			"عوض کردن واژه رمز"); // add for suico theme

/* ------------------- xo_footerstatic.html ------------------- */
define("_THEME_INBOX",                  "نمایش پیام شخصی");
define("_THEME_INBOX_NOTREAD",          "پیام خوانده نشده");
define("_THEME_NOTIFICATIONS",          "نمایش آگاهی رسانی ‌ها");
define("_THEME_PROFILE",                "نمایش ویژگی ‌های فردی");
define("_THEME_ADMINISTRATION",         "مدیریت");
define("_THEME_LOGOUT",                 "برای خروج کلیک کنید");
define("THEME_EDITPROFILE",				"ویرایش ویژگی ‌های فردی");
define("THEME_USER",					"کاربران");
define("THEME_CPHOME",					"بخش مدیریت");
define("THEME_BANS",					"مدیریت بنر ‌ها");
define("THEME_BKAD",					"مدیریت بلاک ‌ها");
define("THEME_ADGS",					"مدیریت گروه ‌ها");
define("THEME_PREF",					"ویژگی ‌ها");
define("THEME_INSTALLEDMODULES",		"مدیریت ماژول ‌ها");
define("THEME_IMAGES",					"تصاویر");
define("THEME_MDAD",					"ماژول ‌ها");
define("THEME_SMLS",					"مدیریت لبخند ‌ها");
define("THEME_RANK",					"نشان اعضا");
define("THEME_EUSER",					"ویرایش اعضا");
define("THEME_FINDUSER",				"جستجو اعضا");
define("THEME_MLUS",					"ارسال پیام با اعضا");
define("THEME_AVATARS",					"مدیریت تصاویر اعضا");
define("THEME_TPLS",					"مدیریت الگو ‌ها");
define("THEME_COMMENTS",				"مدیریت نظر ‌ها");
define("THEME_ADMTOOLS",				"ابزار‌ها");Tools // add for xoops 2.5.0
define("THEME_ADMCLEAN",				"تعمیر");  // add for xoops 2.5.0
define("THEME_SYSSETTING",				"تنظیمات ماژول سیستم");  // add for xoops 2.5.0
define("THEME_UPTOP",					"بالا صفحه"); // add for xoops 2.5.0

?>