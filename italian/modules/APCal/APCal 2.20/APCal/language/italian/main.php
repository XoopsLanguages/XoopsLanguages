<?php

if( defined( 'FOR_XOOPS_LANG_CHECKER' ) || ! defined( 'APCAL_MB_APCALLOADED' ) ) {

define( 'APCAL_MB_APCALLOADED' , 1 ) ;

// index.php
define('_MB_APCAL_ERR_NOPERMTOUPDATE',"Non hai i permessi per modificare gli eventi");
define('_APCAL_APURL', 'http://xoops.antiquespromotion.ca');
define('_APCAL_APURL2', 'http://www.antiquespromotion.ca');
define('_AM_APCAL_COPYRIGHT' , '<a href="http://xoops.antiquespromotion.ca" title="Calendar for Xoops" target="_blank">APCal</a> by <a href="http://www.antiquespromotion.ca" title="Antiques Promotion Canada" target="_blank">AP</a>');
define('_MB_APCAL_ERR_NOPERMTOINSERT',"Non hai i permessi per creare gli eventi");
define('_MB_APCAL_ERR_NOPERMTODELETE',"Non hai i permessi per cancellare gli eventi");
define('_MB_APCAL_ALT_PRINTTHISEVENT',"Stampa questo evento");

// print.php
define('_MB_APCAL_COMESFROM',"Questo evento proviene da %s");
define('_MB_APCAL_PARMALINK',"URL di questo evento");

}

?>
